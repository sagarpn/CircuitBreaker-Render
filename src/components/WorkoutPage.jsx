        ...
        <div className={`${styles.workout} fade-up`}>
          <div className={styles.workoutHeader}>
            <div className={styles.workoutTitleRow}>
              ...
              <button className={styles.copyBtn} onClick={() => {
                ...
                lines.push(`🏋️ ${workoutName}`)
                lines.push('')
                lines.push('CIRCUIT 1')
                lines.push(fmt(workout.circuit1||[]))
                lines.push('')
                lines.push('CIRCUIT 2')
                lines.push(fmt(workout.circuit2||[]))
                if (circuit3) {
                  lines.push('')
                  lines.push('CIRCUIT 3')
                  lines.push(fmt(circuit3))
                }
                lines.push('')
                lines.push('Generated by CircuitBreaker')
                navigator.clipboard?.writeText(lines.join('\n'))
                  .then(() => alert('Workout copied to clipboard!'))
                  .catch(() => alert('Could not copy — try long pressing the text'))
              }}>
                📋 Copy
              </button>
            </div>
          ...
          {workout && (() => {
            ...
            const muscles = [...new Set([...c1ex,...c2ex]
              .filter(e => e.muscle_group)
              .map(e => e.muscle_group.charAt(0).toUpperCase() + e.muscle_group.slice(1))
            )].join(' · ') || (focus === 'upper' ? 'Upper Body' : focus === 'lower' ? 'Lower Body' : focus === 'whole' ? 'Full Body' : 'HIIT')
            return (
              <div className={styles.workoutSummary}>
                <span>{muscles} · {total} exercises</span>
                <span className={styles.summaryExtra}>+ Add Core or Burner for extra results</span>
              </div>
            )
          })()}
